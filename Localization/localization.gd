...
func _load_translations() -> void:
	...
	for column: int in range(1, header.size()):
		...
		if locale_code.begins_with("_") or locale_code.begins_with("?") or locale_code.is_empty():
			continue
		var translation := Translation.new()
		translation.locale = locale_code
		translations_by_locale[column] = translation
	...
	while not file.eof_reached():
		...
		if key.is_empty() or key.begins_with("#"):
			continue
		for column: int in translations_by_locale.keys():
			if column >= row.size():
				continue
			var text := String(row[column])
			(translations_by_locale[column] as Translation).add_message(key, text)

	for translation: Translation in translations_by_locale.values():
