...
const RunFlowUIAdapterScript: GDScript = preload("res://Run/presentation/run_flow_ui_adapter.gd")
...
func _setup_run_scope(
	...
	if not run_scope.initialize(stat_system):
		_discard_run_scope()
		return false
	var dark_marble := preload("res://Content/data/dark_marble.tres") as Item
	var dash_skill := preload("res://Content/data/dash_skill.tres") as Item
