...
- **配置**：`level_values=[2,4,7]`，`extra.awakened_bonus=10`。
...
> 若绿弹珠已把上限升到 20，则 Lv1/Lv2/Lv3/觉醒分别为 22/24/27/30；感染阈值仍为 4，更高上限意味着 DoT 在更高层数才封顶。
...
觉醒 = 等级 ≥ 4（`effect_manager.gd`），`set_level` 仍按 `max_level=3` 钳制，故每件为 **Lv1/Lv2/Lv3/觉醒** 四档。

`parasite` 与 `venom_knife` 均使用 `[1,2,3] + awakened1`，使每个等级提供可见成长；`witch_hat` 使用 `[2,4,7] + awakened10`，在基础上限 10 时为 12/14/17/27，在绿弹珠上限 20 时为 22/24/27/30（硬顶）。

仍待确认：是否把本文与 `archetypes/plague.md`（v4 阶段制幻想）合并或互相指引，避免双份瘟疫文档漂移。
...
| parasite | [1,2,3] | awakened_bonus=1 |
| pustule | [2,1,0] | base_count=1, awakened_count=2 |
| scorpion_tail | [1,2,3] | awakened_bonus=1 |
| venom_knife | [1,2,3] | awakened_bonus=1 |
| witch_hat | [2,4,7] | awakened_bonus=10 |
