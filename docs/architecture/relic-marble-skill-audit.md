...
| `brown_marble` | BROWN | 20 | 2 | 敌人碰撞为左右挡板共享蓄力（上限 2 层）；挡板弹起消费蓄力武装下次命中附伤 |
...
| 大地 | 回响附伤 2 | 回响附伤 4 | 附伤 8 + 挡板弹起球速 ×2 |
...
- 回响：敌人碰撞为左右挡板共享蓄力（上限 2 层，黄/红遮罩）；挡板弹起消费 1 层，
  追加球速（`echo_flipper_speed_multiplier`，默认 1.5）并武装 1 个待结算伤害 token，
  下次敌人命中消费 token 附带回响伤害。控制器预置于 `table_base.tscn`，换桌清零、掉球保留。
...
4. 大地弹珠 + 挡板：命中敌人为挡板蓄力，挡板弹起消费蓄力强化球速与下次命中附伤。
...
- MarbleChain 的实际总伤害、回响 token 消费、炸弹、链首排序；
